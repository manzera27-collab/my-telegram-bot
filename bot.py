...
BOOK_PATH = os.environ.get("KEYTOFATE_BOOK_PATH", "KeytoFate_lesen.txt")
...
def reduzieren(n: int, keep_master: bool = MEISTER_ERHALTEN) -> int:
    """Reduktion mit möglichem Erhalt von Meisterzahlen (11/22/33)."""
    while n > 9:
...
def reduzieren_1_9(n: int) -> int:
    """Harte Reduktion 1–9 (ohne Meisterzahlen) — für Tagesenergie/Partnerschaft/Kollektiv."""
    while n > 9:
...
def tagesenergie(bday_day: int, today_day: int) -> int:
    # Geburtstagstag + heutiger Tag → Ziffernsumme → 1–9
...
# ---------------------- Тексты (встроенные + загрузка из книги) ----------------------

# Короткие аннотации для Geisteszahl (fallback, если нет «Mehr lesen»)
GEISTES_TXT: Dict[int,str] = {
    1: "Die Eins ist Beginn und Führung. Aufgabe: Stärke in verantwortungsvolle Leitung verwandeln.",
    2: "Die Zwei ist Beziehung und Diplomatie. Aufgabe: Brücken bauen, Grenzen setzen.",
    3: "Die Drei ist Kreativität und Wissen. Aufgabe: Inspiration in Wirkung verwandeln.",
    4: "Die Vier ist Struktur und Ausdauer. Aufgabe: Stabilität ohne Starrheit.",
    5: "Die Fünf ist Bewegung und Vielfalt. Aufgabe: Freiheit bewusst leben.",
    6: "Die Sechs ist Liebe und Verantwortung. Aufgabe: Balance zwischen Fürsorge und Weisheit.",
    7: "Die Sieben ist Analyse und Wahrheit. Aufgabe: Erkenntnisse teilen.",
    8: "Die Acht ist Macht und Gerechtigkeit. Aufgabe: Erfolg als Dienst.",
    9: "Die Neun ist Vollendung und Dienst. Aufgabe: Menschlichkeit leben.",
}

# Полные тексты Geisteszahl (кнопка «Mehr lesen») — можешь заменить на буквальные из книги
GEISTES_FULL_TXT: Dict[int,str] = {
    1: "Volltext Geisteszahl 1 (ersetzen durch Wortlaut aus dem Buch).",
    2: "Volltext Geisteszahl 2 (ersetzen durch Wortlaut aus dem Buch).",
    3: "Volltext Geisteszahl 3 (ersetzen durch Wortlaut aus dem Buch).",
    4: "Volltext Geisteszahl 4 (ersetzen durch Wortlaut aus dem Buch).",
    5: "Volltext Geisteszahl 5 (ersetzen durch Wortlaut aus dem Buch).",
    6: "Volltext Geisteszahl 6 (ersetzen durch Wortlaut aus dem Buch).",
    7: "Volltext Geisteszahl 7 (ersetzen durch Wortlaut aus dem Buch).",
    8: "Volltext Geisteszahl 8 (ersetzen durch Wortlaut aus dem Buch).",
    9: "Volltext Geisteszahl 9 (ersetzen durch Wortlaut aus dem Buch).",
}

# ВАЖНОЕ НОВОЕ: Точечные описания по конкретному дню рождения (1..31)
# Заполни дословно из книги для всех 1..31 — либо положи файл книги, и парсер ниже подтянет автоматически.
DAY_BIRTH_TXT: Dict[int,str] = {
    # Примеры — замени на дословный текст из книги
...
Geborene am 16. geboren sind, wird die Energie ihres Bewusstseins immer durch Versuchungen und schädliche Neigungen prüfen. Jede Askese stärkt Sie, aber Sie
müssen Willenskraft und Unverwundbarkeit gegenüber Ihren eigenen Wünschen entwickeln. Auch ist es sehr wichtig für Sie, zu lernen, jeden Ihrer Tage zu planen, langfristige Ziele zu setzen und alle Ihre Angelegenheiten zu Ende zu bringen. Dies wird Ihre Persönlichkeit stärker und größer machen.""",
...
Insgesamt ist das Thema Partnerschaft für Sie von großer Bedeutung – gerade in der Zusammenarbeit mit anderen Menschen können Sie die besten Ergebnisse in Ihrem Leben erreichen. Dafür ist es notwendig, sich selbst und die Psychologie anderer Menschen zu verstehen""",
...
Geborene am 27. geboren sind, haben eine Leidenschaft für spirituelle Suche. Oft verneinen solche Menschen einfach die materielle Welt oder leben im Chaos, da die Energie der Zahlen 2 und 7 viele Zweifel und eine Loslösung von der realen Welt schafft. Ihre wirkliche Aufgabe ist es, sich in Partnerschaft mit anderen Menschen weiterzuentwickeln und komplexe Aufgaben zu lösen. Das ist Ihre Art, der Welt zu dienen.""",
...
# Tagesenergie (краткие тексты — можно расширить из книги)
TAG_TXT: Dict[int,str] = {
    1: "Heute: Neubeginn, klare Entscheidungen, erste Schritte.",
    2: "Heute: Dialog, Ausgleich, Partnerschaft.",
    3: "Heute: Wissen, Kommunikation, Gestaltung.",
    4: "Heute: Struktur, Ordnung, disziplinierte Arbeit.",
    5: "Heute: Veränderung, Chancen, Vernetzung.",
    6: "Heute: Harmonie, Familie, Schönheit.",
    7: "Heute: Analyse, Reflexion, Stille.",
    8: "Heute: Zielklarheit, Management, Ergebnisse.",
    9: "Heute: Abschluss, Dienst, Großzügigkeit.",
}

# Partnerschaft
PARTNERSCHAFT_TXT: Dict[int,str] = {
    1: ("Zwei Führungsenergien bringen Funken, Tempo und große Schaffenskraft. "
        "Mit gemeinsamen Zielen entsteht ein starkes Team; ohne sie drohen Machtspiele."),
    2: ("Zart, empathisch, harmonieorientiert. Diese Verbindung heiligt das Gespräch und liebt Ausgleich. "
        "Ehrlichkeit und Grenzen schützen Nähe."),
    3: ("Lebendig, inspirierend, voller Kommunikation und Lernen. "
        "Struktur und klare Prioritäten verhindern Zerstreuung."),
    4: ("Praktisch und stabil. Ordnung, Disziplin und Beständigkeit prägen das Zusammensein. "
        "Raum für Spontaneität einplanen."),
    5: ("Kommunikativ, beweglich, abenteuerlustig. Offen für neue Erfahrungen. "
        "Innerer Anker und gemeinsame Werte halten Fokus."),
    6: ("Liebe, Fürsorge, Verantwortung. Wärme, Harmonie und Wunsch nach Familie. "
        "Balance zwischen Nähe und Freiheit pflegen."),
    7: ("Tiefe, Transformation, innere Arbeit. Disziplin, Austausch und Rituale beugen Rückzug vor."),
    8: ("Machtvoll, zielorientiert, ergebnisstark. Transparenz, Ethik und Fairness sind Schlüssel."),
    9: ("Reif, sinnstiftend, überpersönlich. Klare Grenzen, Balance Geben/Empfangen."),
}

# Kollektivenergie
KOLLEKTIV_TXT: Dict[int,str] = {
    1: "Initiativen, starke Persönlichkeiten, Führung. Gemeinsame Vision bündeln, Rollen klären.",
    2: "Verbindend, ausgleichend, Wir-Gefühl. Verantwortlichkeiten verankern, ehrlich sprechen.",
    3: "Austausch, Ideen, Lernen. Prioritäten und Prozesse vermeiden Überladung.",
    4: "Strukturiert, ausdauernd, stabil. Innovation zulassen, nicht erstarren.",
    5: "Beweglich, chancenorientiert, Netzwerke. Inneren Kompass und Ziele definieren.",
    6: "Sorgend, wertorientiert, ästhetisch. Faire Lastenverteilung, Balance Nähe/Freiheit.",
    7: "Forschend, diszipliniert, tief. Ergebnisse teilen, Wissen praktisch anwenden.",
    8: "Leistungsstark, zielorientiert, Management. Transparenz und Ethik für Vertrauen.",
    9: "Sinnstiftend, humanitär, abschließend. Grenzen wahren, Erholung kultivieren.",
}

# Entwicklungspfad & Zu vermeiden (словесно, без формулы)
ENTWICKLUNGSPFAD: Dict[int,str] = {
    1: "Zur 4 — über 2 und 3: aus Impuls werden Disziplin und Struktur.",
    2: "Zur 5 — über 3 und 4: Harmonie wird zu bewusster Freiheit.",
    3: "Zur 6 — über 4 und 5: Kreativität wird zu reifer Verantwortung.",
    4: "Zur 7 — über 5 und 6: Ordnung wird zu innerer Weisheit.",
    5: "Zur 8 — über 6 und 7: Liebe/Verantwortung → Wahrheit/Disziplin → gerechter Erfolg.",
    6: "Zur 9 — über 7 und 8: Tiefgang → Macht/Erfolg → universelle Liebe & Dienst.",
    7: "Zur 1 — über 8 und 9: Disziplin & Macht → Abschluss & Dienst → reife Führung.",
    8: "Zur 2 — über 9 und 1: von Macht zu Kooperation und Diplomatie.",
    9: "Zur 3 — über 1 und 2: Dienst & Vollendung → leichter, schöpferischer Ausdruck.",
}
ZU_VERMEIDEN: Dict[int,str] = {
...
# ---------- Автозагрузка из книги (если положишь рядом файл .txt) ----------
def _compile_block_regex(title_variants: List[str], number_group=True) -> re.Pattern:
    titles = "|".join([re.escape(t) for t in title_variants])
    if number_group:
        return re.compile(
            rf"(?P<title>{titles})\s*[:\-]?\s*(?P<num>[1-9][0-9]?)\b\s*(?P<body>.*?)(?=(?:{titles})\s*[:\-]?\s*[0-9]+\b|$)",
            re.IGNORECASE | re.DOTALL
        )
    else:
        return re.compile(
            rf"(?P<title>{titles})\s*(?P<body>.*?)(?=(?:{titles})\b|$)",
            re.IGNORECASE | re.DOTALL
        )
...
def load_book_texts_if_present():
    path = BOOK_PATH
    ...
    try:
        ...
        # Geisteszahl 1–9 (полные тексты)
        geist_rx = _compile_block_regex(["Geisteszahl","GEISTESZAHL"])
        for m in geist_rx.finditer(content):
            num = int(m.group("num"))
            if 1 <= num <= 9:
                GEISTES_FULL_TXT[num] = _normalize_text_block(m.group("body"))

        # Tagesenergie 1–9
        tag_rx = _compile_block_regex(["Tagesenergie","Tages-Energie","Persönlicher Tag","Personlicher Tag"])
        for m in tag_rx.finditer(content):
            num = int(m.group("num"))
            if 1 <= num <= 9:
                TAG_TXT[num] = _normalize_text_block(m.group("body"))

        # Geburtstagsbedeutung 1–31 (ищем по заголовкам «Geburtstag», «Geburtstagszahl», «Tag des Monats»)
        day_rx = _compile_block_regex(["Geburtstag","Geburtstagszahl","Tag des Monats","Geburts-Tag"])
        for m in day_rx.finditer(content):
            num = int(m.group("num"))
            if 1 <= num <= 31:
                DAY_BIRTH_TXT[num] = _normalize_text_block(m.group("body"))

        print("[OK] Buchtexte erkannt und geladen (Geisteszahl/Tagesenergie/Geburtstage).")
    except Exception as e:
        print(f"[WARN] Konnte Buchtexte nicht vollständig laden: {e}")

# Пытаемся загрузить тексты из книги (не обязательно — словари уже вшиты)
load_book_texts_if_present()
...
async def read_more_geist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        full = GEISTES_FULL_TXT.get(g, "").strip()
        if not full:
...
async def full_analysis_fallback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Точечный текст по дню рождения
        day_text = DAY_BIRTH_TXT.get(d, "").strip()
        day_block = f"📅 <b>Bedeutung des Geburtstagstages {d}</b>\n{html_escape(day_text)}\n\n" if day_text else ""
