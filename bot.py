...
# ---------- DONATE (только кнопка, без текста внизу сообщений) ----------
...
def ai_answer_from_corpus(query: str) -> str:
    ...
    if not top:
        return "Ich konnte dazu keinen spezifischen Abschnitt in der Lehre finden. Formulieren Sie bitte die Frage präziser."
...
async def ask_full(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Полные тексты: берём из корпуса, если есть
        geist_full = GEISTES_FULL.get(g, "").strip()
        handl_full = HANDLUNGS_FULL.get(h, "").strip()
        verw_full  = VERWIRK_FULL.get(v, "").strip()

        # Сборка ответа
        parts = [
            f"<b>Vollanalyse für {d:02d}.{m:02d}.{y}</b>",
            f"🧠 <b>Geisteszahl:</b> {g}\n{html_escape(GEISTES_TXT_SHORT.get(g,'').strip())}",
        ]
        if geist_full:
            parts.append(f"{html_escape(geist_full)}")

        parts.append(f"⚡ <b>Handlungszahl:</b> {h}\n{HANDLUNG_SHORT[(h-1)%9]}")
        if handl_full:
            parts.append(f"{html_escape(handl_full)}")

        parts.append(f"🎯 <b>Verwirklichungszahl:</b> {v}\n{VERWIRK_SHORT[(v-1)%9]}")
        if verw_full:
            parts.append(f"{html_escape(verw_full)}")

        parts.append(f"📘 <b>Ergebniszahl:</b> {e}\n{ERGEBNIS_SHORT[(e-1)%9]}")
