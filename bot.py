# -*- coding: utf-8 -*-
# === KeyToFate Bot — PART 1/3 ===
from __future__ import annotations
import os, re
from typing import Dict

# Путь к книге (можно переопределить переменной окружения K2_PATH)
...
def parse_date(text: str) -> Tuple[int,int,int]:
    ...
    if not m: raise ValueError("Bitte Datum im Format TT.MM.JJJJ, z. B. 25.11.1978.")
...
def parse_dates_multi(text: str) -> List[Tuple[int,int,int]]:
    ...
    for d,mth,yr in found:
        datetime(year=int(yr), month=int(mth), day=int(d))
        result.append((int(d),int(mth),int(yr)))
    return result

def reduzieren(n:int,keep_master=True)->int:
    while n>9:
        s=sum(int(d) for d in str(n))
        if keep_master and s in (11,22,33): return s
        n=s
    return n
...
# -*- coding: utf-8 -*-
# === KeyToFate Bot — PART 3/3 ===

async def on_menu_click(update:Update,context:ContextTypes.DEFAULT_TYPE):
    q=update.callback_query;data=q.data;await q.answer()
    if data=="calc_full": await q.message.reply_html("🧮 Geben Sie Geburtsdatum ein (TT.MM.JJJJ):");return ASK_FULL
    if data=="calc_day": await q.message.reply_html("Geben Sie Ihr Geburtsdatum ein (TT.MM.JJJJ):");return ASK_DAY_BIRTH
    if data=="calc_compat": await q.message.reply_html("Geben Sie Geburtsdatum Person 1 ein (TT.MM.JJJJ):");return ASK_COMPAT_1
    if data=="calc_name": await q.message.reply_html("Geben Sie den Namen ein (lateinische Schreibweise):");return ASK_NAME
    if data=="calc_group": context.user_data["group_birthdays"]=[];await q.message.reply_html("👥 Bis zu 5 Geburtstage eingeben. Schreiben Sie <b>fertig</b>.");return ASK_GROUP
    if data=="calc_path": await q.message.reply_html("🧭 Bitte Geburtsdatum eingeben (TT.MM.JJJJ):");return ASK_PATH
    return ConversationHandler.END

# --- Handlers (пример Vollanalyse с кнопкой Zurück) ---
async def ask_full(update:Update,context:ContextTypes.DEFAULT_TYPE):
    try:
        ...
        v=verwirklichungszahl(g,h);e=ergebniszahl(g,h,v);geld=geldcode(d,m,y)
        geist_short=GEISTES_TXT.get(g,"");geist_full=get_geistes(g)
        handl_full=get_handlungs(h);verw_full=get_verwirk(v);erg_full=get_ergebnis(e)
        ...
        if day_text: parts.append(f"\n\n📅 <b>Bedeutung des Tages:</b>\n{html_escape(day_text)}")
        kb=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Zurück zum Menü",callback_data="open_menu")]])
        await update.message.reply_html("\n\n".join(parts),reply_markup=kb)
        return ConversationHandler.END
    except Exception as ex:
        await update.message.reply_html(f"❌ Fehler: {html_escape(str(ex))}");return ASK_FULL

# --- остальные ask_xxx делаются аналогично (mit Zurück-Knopf) ---
...
def main():
    ...
    conv=ConversationHandler(
        ...
        states={
          ...
          # остальные тоже: ASK_DAY_BIRTH, ASK_COMPAT_1, ASK_COMPAT_2 и т.д.
        },
